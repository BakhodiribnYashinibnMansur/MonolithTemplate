...
func generateNewAccessToken(id, roleID, roleTitle string) (string, error) {
	...
	// Set public claims:
	claims["id"] = id
	claims["expire"] = time.Now().Add(time.Minute * time.Duration(conf.JWT.JWTSecretKeyExpireMinutes)).Unix()
	...
	if conf.Server.Environment == "develop" {
		claims["expire"] = time.Now().Add(time.Minute * time.Duration(100*conf.JWT.JWTSecretKeyExpireMinutes)).Unix()
	} else {
